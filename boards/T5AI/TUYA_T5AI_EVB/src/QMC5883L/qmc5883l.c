...
void qmc5883l_device_init(){
    qmc5883l_write_reg(QMC5883L_ADDRESS,QMC5883L_REG_CONTROL,odr|range|sample_rate|mode);// 详见寄存器地址0x09的存储四个数值介绍
}
...
{
    // uint8_t data[6];
    // tkl_i2c_read(I2C_NUMBER,QMC5883L_ADDRESS,0x00,data,6);
    // TODO: Add Security Check!

    uint8_t value;
    tkl_i2c_master_send(I2C_NUMBER,addr,&reg,1,TRUE);
    tkl_i2c_master_receive(I2C_NUMBER,addr,&value,1,FALSE);
    return value;
}
...
void qmc5883l_reset_calibration(void){
    mag_calibration.x_min = -32768;
    mag_calibration.x_max = 32767;
    mag_calibration.y_min = -32768;
    mag_calibration.y_max = 32767;
    mag_calibration.z_min = -32768;
    mag_calibration.z_max = 32767;
}

void qmc5883l_update_calibration(float x, float y, float z){
    if(x < mag_calibration.x_min){
        mag_calibration.x_min = x;
    }
    if(x > mag_calibration.x_max){
        mag_calibration.x_max = x;
    }
    if(y < mag_calibration.y_min){
        mag_calibration.y_min = y;
    }
    if(y > mag_calibration.y_max){
        mag_calibration.y_max = y;
    }
    if(z < mag_calibration.z_min){
        mag_calibration.z_min = z;
    }
    if(z > mag_calibration.z_max){
        mag_calibration.z_max = z;
    }
}
...
void qmc5883l_calibration(float* x, float* y, float* z){
    ...
    // 校准坐标和范围
    float offset_x = (mag_calibration.min_x + mag_calibration.max_x) / 2.0f;
    float offset_y = (mag_calibration.min_y + mag_calibration.max_y) / 2.0f;
    float offset_z = (mag_calibration.min_z + mag_calibration.max_z) / 2.0f;
    float range_x = mag_calibration.max_x - mag_calibration.min_x;
    float range_y = mag_calibration.max_y - mag_calibration.min_y;
    float range_z = mag_calibration.max_z - mag_calibration.min_z;
    float avg_range = (range_x + range_y) / 2.0f;// 不计算z轴
    float scale_x = avg_range / range_x;
    float scale_y = avg_range / range_y;
    float scale_z = 1.0f; // Z轴通常不需要缩放
